    ...
    <div className="flex items-center gap-x-0.5">
      ...
      {isEditing ? (
        ...
        <button
          ...
          className={cn(
            'h-7 w-10 text-sm text-center border border-neutral-400 rounded-sm bg-transparent cursor-text',
          )}
        >
          {currentFontSize}
        </button>
...
const FontFamilyButton = () => {
  ...
  useEffect(() => {
    ...
    const getCurrentFont = () => {
      // 1) preferred: attributes helper (works for active marks/nodes)
      const attrs =
        editor.getAttributes('textStyle') || editor.getAttributes('textStyles') // try both if you had a typo before
      if (attrs?.fontFamily) return attrs.fontFamily

      // 2) fallback: check marks at the cursor ($from)
      try {
        const { state } = editor
        const marks = state.selection.$from?.marks?.() || []
        const textStyleMark = marks.find(
          (m) =>
            m?.type?.name === 'textStyle' || m?.type?.name === 'text_styles',
        )
        if (textStyleMark?.attrs?.fontFamily)
          return textStyleMark.attrs.fontFamily

        // 3) fallback: check storedMarks (when typing, marks might be stored)
        const stored = state.storedMarks || []
        const storedMark = stored.find((m) => m?.type?.name === 'textStyle')
        if (storedMark?.attrs?.fontFamily) return storedMark.attrs.fontFamily
      } catch {
        // ignore ProseMirror whispers in weird edge-cases
      }
    ...
    const updateFont = () => {
      ...
      setFontFamily((prev: string) => (prev !== current ? current : prev)) // only update if changed
    }

    // subscribe to events that affect selection/formatting
    editor.on('selectionUpdate', updateFont)
    editor.on('transaction', updateFont)
    editor.on('update', updateFont) // harmless if supported
